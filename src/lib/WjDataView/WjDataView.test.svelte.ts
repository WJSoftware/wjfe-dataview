...
import WjDataView, { defineData, GridLines, type ColAlignment, type ControlColumn, type DataCellContext, type WjDvColumn, type WjDvRow } from './WjDataView.svelte';
...
describe('WjDataView', () => {
    ...
    test("Should pass any unknown properties as attributes to the table element.", () => {
        // Arrange.
        const columns = $state<WjDvColumn[]>([
            {
                key: 'id',
        ...
        // Act.
        const { getByRole } = render(WjDataView, { columns, data, 'data-test': 'abc' });

        // Assert.
        const table = getByRole('table');
        expect(table.getAttribute('data-test')).toEqual('abc');
    });
